...
import { DiscoveryLayerSystem, type DiscoveryLayer, type DiscoveryProgress } from './DiscoveryLayerSystem';
...
export const DiscoveryWorld: React.FC<DiscoveryWorldProps> = ({
  ...
  const [discoveryAchievements, setDiscoveryAchievements] = useState<Array<{
    id: string;
    type: string;
  ...
  function handleArtifactDiscovered(artifact: any, layer: DiscoveryLayer) {
    ...
    updateConsciousness(prev => ({
      ...prev,
      awarenessLevel: Math.min(1.0, prev.awarenessLevel + consciousnessBoost),
    }));
  }
  ...
  const getCameraPosition = useMemo(() => {
    ...
    return layerPositions[discoverySystem.progress.currentLayer];
    ...
    <div className="relative w-full h-screen bg-gradient-to-b from-black via-indigo-950 to-purple-950 overflow-hidden">
      ...
      >
        ...
        <PortalChamberScene
          ...
          onConsciousnessEvolution={onConsciousnessEvolution}
        />
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute top-4 left-4 text-white/80 font-mono text-sm space-y-1">
          ...
          <div>Layer: {discoverySystem.progress.currentLayer} - {discoverySystem.currentLayerConfig.name}</div>
        ...
        <div className="absolute bottom-4 left-4 space-y-2">
          {discoveryAchievements.slice(-3).map((achievement) => (
            ...
            >
              ...
              <div>Layer {achievement.layer}: {achievement.data.name || achievement.type}</div>
            </div>
          ))}
        </div>
