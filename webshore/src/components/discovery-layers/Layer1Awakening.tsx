...
export const Layer1Awakening: React.FC<Layer1AwakeningProps> = ({
      ...
      if (!symbol.discovered) {
        ...
        if (distance < discoveryRange) {
          ...
          setSymbolGarden(prev => ({
            symbols: prev.symbols.map((s, i) => 
              i === index ? { ...s, discovered: true } : s
            ),
          }));
  ...
  const activateSymbol = (symbolId: string, interactionPosition: Vector3) => {
    ...
    if (!symbol.discovered || symbol.activated) return;
    ...
    if (distance < 2) {
      setSymbolGarden(prev => ({
        symbols: prev.symbols.map((s, i) => 
          i === symbolIndex ? { ...s, activated: true } : s
        ),
      }));
  ...
  useFrame((state, delta) => {
      ...
      children.forEach((child, index) => {
        if (child instanceof Points) {
          ...
          for (let i = 0; i < positions.length; i += 3) {
            ...
            positions[i + 1] += floatOffset * delta;
            
            // Spiral motion around symbols
            const angle = time * 0.1 + particleIndex * 0.01;
            positions[i] += Math.cos(angle) * delta * 0.01;
            positions[i + 2] += Math.sin(angle) * delta * 0.01;
          }
    ...
    symbolGarden.symbols.forEach((symbol, index) => {
      if (symbol.discovered) {
        const symbolMesh = groupRef.current?.children.find(child => 
          child.userData.symbolId === symbol.id
        ) as Mesh;
        ...
        <group key={symbol.id} position={symbol.position.toArray()}>
          {symbol.discovered && (
            <mesh
              ...
              onClick={(e) => activateSymbol(symbol.id, new Vector3().setFromMatrixPosition(e.object.matrixWorld))}
            >
