...
const ENNEAGRAM_TYPES: Omit<EnneagramType, 'strength'>[] = [
  { number: 1, name: 'Perfectionist', center: 'body', position: new Vector3(0, 2, 0), color: new Color('#FF4444'), wing1: 9, wing2: 2, arrow: 7, stress: 4, security: 7 },
  { number: 2, name: 'Helper', center: 'heart', position: new Vector3(1.5, 1, 0), color: new Color('#FF8844'), wing1: 1, wing2: 3, arrow: 8, stress: 8, security: 4 },
  { number: 3, name: 'Achiever', center: 'heart', position: new Vector3(2, 0, 0), color: new Color('#FFAA44'), wing1: 2, wing2: 4, arrow: 9, stress: 9, security: 6 },
  { number: 4, name: 'Individualist', center: 'heart', position: new Vector3(1.5, -1, 0), color: new Color('#FFDD44'), wing1: 3, wing2: 5, arrow: 1, stress: 2, security: 1 },
  { number: 5, name: 'Investigator', center: 'head', position: new Vector3(0, -2, 0), color: new Color('#AAFF44'), wing1: 4, wing2: 6, arrow: 8, stress: 7, security: 8 },
  { number: 6, name: 'Loyalist', center: 'head', position: new Vector3(-1.5, -1, 0), color: new Color('#44FF44'), wing1: 5, wing2: 7, arrow: 9, stress: 3, security: 9 },
  { number: 7, name: 'Enthusiast', center: 'head', position: new Vector3(-2, 0, 0), color: new Color('#44FFAA'), wing1: 6, wing2: 8, arrow: 5, stress: 1, security: 5 },
  { number: 8, name: 'Challenger', center: 'body', position: new Vector3(-1.5, 1, 0), color: new Color('#44AAFF'), wing1: 7, wing2: 9, arrow: 2, stress: 5, security: 2 },
  { number: 9, name: 'Peacemaker', center: 'body', position: new Vector3(0, 2, 0), color: new Color('#4444FF'), wing1: 8, wing2: 1, arrow: 3, stress: 6, security: 3 },
];

const ENNEAGRAM_CENTERS: EnneagramCenter[] = [
  { name: 'body', types: [1, 8, 9], color: new Color('#FF6B6B'), position: new Vector3(0, 1, 0), element: 'earth' },
  { name: 'heart', types: [2, 3, 4], color: new Color('#4ECDC4'), position: new Vector3(1.5, -0.5, 0), element: 'water' },
  { name: 'head', types: [5, 6, 7], color: new Color('#45B7D1'), position: new Vector3(-1.5, -0.5, 0), element: 'air' },
];
...
export const EnneagramEngine: React.FC<EnneagramEngineProps> = ({
  ...
  const { primaryType, wings, arrows, typeStrengths, centerGeometries } = useMemo(() => {
    ...
    const enneagramData = state.data;
    ...
    const centerGeos = ENNEAGRAM_CENTERS.map(center => {
      return generateSacredGeometry({
        type: center.element === 'earth' ? 'cube' : 
              center.element === 'water' ? 'icosahedron' : 'octahedron',
        radius: 0.5,
  ...
  const typeFractals = useMemo(() => {
    ...
    ENNEAGRAM_TYPES.forEach(type => {
      ...
      if (strength > 0.2) { // Only generate fractals for significant types
        const fractal = createFractalGeometry({
          type: type.center === 'body' ? 'mandelbrot' :
                type.center === 'heart' ? 'julia' : 'sierpinski',
  ...
  return (
    <group ref={groupRef} position={position}>
      ...
      {ENNEAGRAM_TYPES.map((type) => {
        ...
        return (
          ...
          >
            ...
            <mesh>
              <sphereGeometry args={[
                isPrimary ? 0.25 : isWing ? 0.15 : 0.1,
                16, 16
              ]} />
            ...
            {typeFractals.has(type.number) && (
              <mesh
                geometry={typeFractals.get(type.number)}
                scale={[0.5, 0.5, 0.5]}
              >
                <meshStandardMaterial
                  color={type.color}
      ...
      {/* Wing Connections */}
      {primaryType && wings.map((wing, index) => {
      ...
      {/* Arrow Connections */}
      {primaryType && arrows.map((arrow, index) => {
