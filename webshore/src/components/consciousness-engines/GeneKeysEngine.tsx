...
const CODON_FREQUENCIES: Record<string, number> = {
  'UUU': 528, 'UUC': 594, 'UUA': 396, 'UUG': 417,
  'UCU': 528, 'UCC': 639, 'UCA': 741, 'UCG': 852,
  'UAU': 963, 'UAC': 174, 'UAA': 285, 'UAG': 396,
  'UGU': 528, 'UGC': 639, 'UGA': 741, 'UGG': 852,
  // ... (simplified set for demo)
};

const AMINO_ACID_COLORS: Record<string, Color> = {
  'Phe': new Color('#FF6B6B'), 'Leu': new Color('#4ECDC4'),
  'Ser': new Color('#45B7D1'), 'Tyr': new Color('#96CEB4'),
  'Cys': new Color('#FFEAA7'), 'Trp': new Color('#DDA0DD'),
  'Pro': new Color('#98D8C8'), 'His': new Color('#F7DC6F'),
  'Gln': new Color('#BB8FCE'), 'Arg': new Color('#85C1E9'),
  'Ile': new Color('#F8C471'), 'Met': new Color('#82E0AA'),
  'Thr': new Color('#F1948A'), 'Asn': new Color('#85C1E9'),
  'Lys': new Color('#F7DC6F'), 'Val': new Color('#A9DFBF'),
  'Ala': new Color('#D7BDE2'), 'Asp': new Color('#AED6F1'),
  'Glu': new Color('#A3E4D7'), 'Gly': new Color('#FADBD8'),
};
...
export const GeneKeysEngine: React.FC<GeneKeysEngineProps> = ({
  ...
  const { dnaStrands, activationSequence, consciousnessLevels } = useMemo(() => {
    ...
    geneKeys.forEach((geneKey, index) => {
      ...
      // First strand
      strand1Points.push(new Vector3(
        Math.cos(angle) * radius,
        height,
        Math.sin(angle) * radius
      ));
      ...
      strand2Points.push(new Vector3(
  ...
  return (
    <group ref={groupRef} position={position}>
      ...
      {activationSequence.map((geneKey, index) => (
        <group
          key={`genekey-${geneKey.gate}`}
          position={geneKey.position.toArray()}
          userData={{ geneKey }}
        >
          {/* Codon sphere */}
          <mesh>
          ...
          <mesh position={[0, -0.2, 0]}>
            ...
            <meshStandardMaterial color={SPHERE_COLORS[geneKey.sphere]} />
          </mesh>
          
          {/* Codon fractal pattern */}
          {codonFractals[index] && (
            <mesh
              geometry={codonFractals[index]}
              scale={[0.5, 0.5, 0.5]}
            >
              <meshStandardMaterial
                color={geneKey.color}
      ...
      {consciousnessLevels.map((level, index) => (
        ...
        >
          ...
          <meshBasicMaterial
            color={level.color}
            transparent
            opacity={level.opacity * consciousnessLevel}
          />
        </mesh>
      ))}
      
      {/* Base pairs connections */}
      {dnaStrands.length === 2 && dnaStrands[0].geneKeys.map((geneKey1, index) => {
