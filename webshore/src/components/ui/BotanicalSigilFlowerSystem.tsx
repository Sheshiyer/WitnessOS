...
export const BotanicalSigilFlowerSystem: React.FC<BotanicalSigilFlowerSystemProps> = ({
  ...
  const archetypalHues = useMemo<ArchetypalHue[]>(() => [
    { name: 'wisdom', color: new THREE.Color('#4A90E2'), frequency: 528, meaning: 'Deep knowing and insight' },
    { name: 'love', color: new THREE.Color('#E91E63'), frequency: 639, meaning: 'Heart connection and compassion' },
    { name: 'transformation', color: new THREE.Color('#9C27B0'), frequency: 741, meaning: 'Change and evolution' },
    { name: 'healing', color: new THREE.Color('#4CAF50'), frequency: 417, meaning: 'Restoration and renewal' },
    { name: 'intuition', color: new THREE.Color('#673AB7'), frequency: 852, meaning: 'Inner guidance and perception' },
    { name: 'creativity', color: new THREE.Color('#FF9800'), frequency: 285, meaning: 'Expression and innovation' },
    { name: 'grounding', color: new THREE.Color('#795548'), frequency: 396, meaning: 'Stability and presence' },
    { name: 'clarity', color: new THREE.Color('#00BCD4'), frequency: 963, meaning: 'Clear vision and understanding' },
  ], []);
  ...
  // Generate sigil flower based on consciousness state
  ...
  const sigilMaterial = useMemo(() => {
    return new THREE.ShaderMaterial({
      uniforms: {
        ...
        archetypalColor: { value: archetypalHues.find(h => h.name === selectedArchetype)?.color || new THREE.Color('#4A90E2') },
        bloomPhase: { value: 0 },
      },
      vertexShader: `
        uniform float time;
  ...
  useFrame((state, delta) => {
    ...
    // Update sigil flowers bloom phases
    setSigilFlowers(prev => prev.map(sigil => ({
      ...sigil,
