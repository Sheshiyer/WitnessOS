...
export const PortalChamberScene: React.FC<PortalChamberSceneProps> = ({
  ...
  const { isConnected, connectionStatus } = useWitnessOSAPI();
  
  // Local state
  const [breathState, setBreathState] = useState<BreathState>({
    pattern: { inhaleCount: 4, holdCount: 4, exhaleCount: 4, pauseCount: 4, totalCycle: 16 },
    phase: 'pause',
    intensity: 0,
    rhythm: 15,
    coherence: 0,
  ...
  const handleBreathStateChange = useCallback((newBreathState: BreathState) => {
    ...
    if (newBreathState.coherence > 0.6) {
      ...
      updateConsciousness(prev => ({
        ...prev,
        awarenessLevel: Math.min(1.0, prev.awarenessLevel + awarenessIncrease),
      }));
    }
  }, [updateConsciousness]);
   ...
   */
  const handleConsciousnessUpdate = useCallback((newConsciousness: ConsciousnessState) => {
  ...
  const getArchetypalColor = (): [number, number, number] => {
    ...
    return typeColors[humanDesignType.toLowerCase()] || typeColors.generator;
  };
    ...
    <div className="relative w-full h-screen bg-gradient-to-b from-indigo-950 via-purple-950 to-black overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute top-4 left-4 text-white/80 font-mono text-sm space-y-1">
          ...
          <div>Breath: {breathState.phase} ({(breathState.coherence * 100).toFixed(1)}%)</div>
          <div>Type: {humanDesignType} | {enneagramType}</div>
          {portalActivated && (
        ...
        {enablePerformanceStats && (
          <div className="absolute top-4 right-4 text-white/60 font-mono text-xs space-y-1">
            ...
            {deviceCapabilities.isLowEnd && (
              <div className="text-yellow-400">Low-End Mode</div>
            )}
          </div>
        )}
          ...
          {!portalActivated ? (
            <>
              <p className="text-lg mb-2 font-light">
                {breathState.coherence < 0.3 ? 'Begin breathing deeply...' :
                 breathState.coherence < 0.7 ? 'Synchronize your breath...' :
                 'Portal energy building...'}
              </p>
              <p className="text-sm opacity-80">
                Your breath activates the consciousness field
              </p>
              {enableBreathDetection && (
                <p className="text-xs mt-2 opacity-60">
                  Microphone enabled for breath detection
                </p>
              )}
            </>
          ) : (
            <div className="text-center">
              <p className="text-xl text-green-400 mb-2 animate-pulse">
                Portal Activated
              </p>
              <p className="text-sm">
                The consciousness field responds to your presence
              </p>
            </div>
          )}
        </div>
