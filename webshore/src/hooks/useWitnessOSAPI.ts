...
import { witnessOSAPI, WitnessOSAPIError, DataTransformer } from '@/utils/api-client';
import type { 
  EngineAPIResponse, 
  EngineName, 
  EngineInput, 
...
interface UseWitnessOSAPIReturn {
  ...
  batchCalculate: (requests: Array<{ engine: EngineName; input: EngineInput }>) => Promise<EngineAPIResponse<EngineOutput[]>>;
...
export const useWitnessOSAPI = (options: UseWitnessOSAPIOptions = {}): UseWitnessOSAPIReturn => {
  ...
  const handleError = useCallback((error: unknown, engine?: string): ConsciousnessError => {
  ...
  const calculateEngine = useCallback(async <TInput extends EngineInput, TOutput extends EngineOutput>(
    engineName: EngineName,
    input: TInput
  ): Promise<EngineAPIResponse<TOutput>> => {
    ...
    try {
      ...
      const transformedInput = autoTransform 
        ? DataTransformer.typeScriptToPython<TInput>(input as Record<string, unknown>)
        : input;
      ...
      if (response.success && response.data) {
        ...
        const transformedData = autoTransform 
          ? DataTransformer.pythonToTypeScript<TOutput>(response.data as Record<string, unknown>)
          : response.data;
  ...
  const calculateNumerology = useCallback((input: any) =>
    calculateEngine('numerology', input), [calculateEngine]);

  const calculateHumanDesign = useCallback((input: any) =>
    calculateEngine('human_design', input), [calculateEngine]);

  const calculateTarot = useCallback((input: any) =>
    calculateEngine('tarot', input), [calculateEngine]);

  const calculateIChing = useCallback((input: any) =>
    calculateEngine('iching', input), [calculateEngine]);

  const calculateEnneagram = useCallback((input: any) =>
    calculateEngine('enneagram', input), [calculateEngine]);

  const calculateSacredGeometry = useCallback((input: any) =>
    calculateEngine('sacred_geometry', input), [calculateEngine]);

  const calculateBiorhythm = useCallback((input: any) =>
    calculateEngine('biorhythm', input), [calculateEngine]);

  const calculateVimshottari = useCallback((input: any) =>
    calculateEngine('vimshottari', input), [calculateEngine]);

  const calculateGeneKeys = useCallback((input: any) =>
    calculateEngine('gene_keys', input), [calculateEngine]);

  const calculateSigilForge = useCallback((input: any) =>
    calculateEngine('sigil_forge', input), [calculateEngine]);
  ...
  const healthCheck = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (connected && response.data?.engines) {
        setAvailableEngines(response.data.engines);
      }
